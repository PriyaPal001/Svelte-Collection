<script>
	...
	import { allhs } from '$lib/components/PageEg/HomeScreen/allhs';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: HSData = allhs.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(HSData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={HSData.comp} />
		</div>
		<div>
