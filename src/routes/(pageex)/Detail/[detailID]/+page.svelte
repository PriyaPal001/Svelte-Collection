<script>
	...
	import { allds } from '$lib/components/PageEg/DetailScreen/allds';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: DSData = allds.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(DSData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={DSData.comp} />
		</div>
		<div>
