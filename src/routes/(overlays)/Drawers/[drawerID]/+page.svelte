<script>
	...
	import { alldraw } from '$lib/components/Overlays/Drawers/alldraw';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: DrawData = alldraw.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(DrawData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={DrawData.comp} />
		</div>
		<div>
