<script>
	...
	import { allnotify } from '$lib/components/Overlays/notify/allnotify';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: NotifyData = allnotify.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(NotifyData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={NotifyData.comp} />
		</div>
		<div>
