<script>
	...
	import { allmd } from '$lib/components/Overlays/modelDialogs/allmd';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: MDData = allmd.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(MDData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={MDData.comp} />
		</div>
		<div>
