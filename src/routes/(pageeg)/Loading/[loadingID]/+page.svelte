<script>
	...
	import { allLoad } from '$lib/components/PageExample/LoadingPage/allLoad';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: LoadData = allLoad.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(LoadData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={LoadData.comp} />
		</div>
		<div>
