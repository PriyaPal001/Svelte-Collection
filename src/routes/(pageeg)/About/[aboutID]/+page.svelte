<script>
	...
	import { allabout } from '$lib/components/PageExample/AboutPage/allabout';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: AboutData = allabout.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(AboutData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={AboutData.comp} />
		</div>
		<div>
