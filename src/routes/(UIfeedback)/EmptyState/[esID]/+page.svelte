<script>
	...
	import { alles } from '$lib/components/UIfeedback/EmptyStates/alles';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: ESData = alles.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(ESData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={ESData.comp} />
		</div>
		<div>
