<script>
	...
	import { allalerts } from '$lib/components/UIfeedback/Alerts/allalerts';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: AlertData = allalerts.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(AlertData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={AlertData.comp} />
		</div>
		<div>
