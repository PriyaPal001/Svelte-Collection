<script>
	...
	import { allTables } from '$lib/components/Lists/tables/alltable';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: TableData = allTables.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(TableData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={TableData.comp} />
		</div>
		<div>
