<script>
	...
	import { allph } from '$lib/components/Headings/PageHead/allph';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: PHData = allph.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(PHData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={PHData.comp} />
		</div>
		<div>
