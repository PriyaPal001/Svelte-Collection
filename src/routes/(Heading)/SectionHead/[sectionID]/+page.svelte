<script>
	...
	import { allshs } from '$lib/components/Headings/SectionHead/allsh';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: SHData = allshs.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(SHData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={SHData.comp} />
		</div>
		<div>
