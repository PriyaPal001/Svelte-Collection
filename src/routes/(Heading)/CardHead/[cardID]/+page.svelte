<script>
	...
	import { allchs } from '$lib/components/Headings/CardHead/allch';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: CHData = allchs.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(CHData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={CHData.comp} />
		</div>
		<div>
