<script>
	...
	import { allproduct } from '$lib/components/EpageEg/Product/allproduct';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: ProductData = allproduct.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(ProductData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={ProductData.comp} />
		</div>
		<div>
