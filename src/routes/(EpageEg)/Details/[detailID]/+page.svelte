<script>
	...
	import { allod } from '$lib/components/EpageEg/Detail/allod';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: OdetailData = allod.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(OdetailData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={OdetailData.comp} />
		</div>
		<div>
