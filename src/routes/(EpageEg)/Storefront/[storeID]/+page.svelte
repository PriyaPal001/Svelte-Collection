<script>
	...
	import { allsf } from '$lib/components/EpageEg/StoreFront/allsf';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: StoreData = allsf.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(StoreData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={StoreData.comp} />
		</div>
		<div>
