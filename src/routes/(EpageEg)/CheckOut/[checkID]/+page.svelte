<script>
	...
	import { allco } from '$lib/components/EpageEg/Checkout/allco';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: CheckOutData = allco.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(CheckOutData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={CheckOutData.comp} />
		</div>
		<div>
