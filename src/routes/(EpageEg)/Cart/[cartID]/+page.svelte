<script>
	...
	import { allcart } from '$lib/components/EpageEg/Cart/allcart';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: CartData = allcart.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(CartData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={CartData.comp} />
		</div>
		<div>
