<script>
	...
	import { allfeed } from '$lib/components/Feedback/404Page/allfeed';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: FeedData = allfeed.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(FeedData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={FeedData.comp} />
		</div>
		<div>
