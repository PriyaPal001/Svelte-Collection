<script>
	...
	import { allSides } from '$lib/components/ApplShell/Sidebar/allside';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: sideData = allSides.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(sideData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={sideData.comp} />
		</div>
		<div>
