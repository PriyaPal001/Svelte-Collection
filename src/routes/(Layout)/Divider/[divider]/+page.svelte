<script>
	...
	import { alldiv } from '$lib/components/Layout/divider/alldiv';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: DivData = alldiv.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(DivData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={DivData.comp} />
		</div>
		<div>
