<script>
	...
	import { alllist } from '$lib/components/Layout/list/alllist';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: ListData = alllist.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(ListData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={ListData.comp} />
		</div>
		<div>
