<script>
	...
	import { allbadge } from '$lib/components/UIelements/badges/allbadge';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: BadgeData = allbadge.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(BadgeData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={BadgeData.comp} />
		</div>
		<div>
