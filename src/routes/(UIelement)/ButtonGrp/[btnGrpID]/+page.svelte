<script>
	...
	import { allbGrp } from '$lib/components/UIelements/buttonGrp/allbGrp';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: BgrpData = allbGrp.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(BgrpData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={BgrpData.comp} />
		</div>
		<div>
