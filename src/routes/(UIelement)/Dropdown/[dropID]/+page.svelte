<script>
	...
	import { alldd } from '$lib/components/UIelements/dropdown/alldd';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: DropData = alldd.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(DropData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={DropData.comp} />
		</div>
		<div>
