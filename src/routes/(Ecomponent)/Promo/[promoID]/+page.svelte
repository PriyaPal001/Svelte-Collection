<script>
	...
	import { allpromo } from '$lib/components/ECompo/Promo/allpromo';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: PromoData = allpromo.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(PromoData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={PromoData.comp} />
		</div>
		<div>
