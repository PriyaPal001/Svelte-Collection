<script>
	...
	import { allsummary } from '$lib/components/ECompo/Summary/allsummary';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: SummaryData = allsummary.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(SummaryData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={SummaryData.comp} />
		</div>
		<div>
