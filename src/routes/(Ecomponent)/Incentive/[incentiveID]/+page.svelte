<script>
	...
	import { allincentive } from '$lib/components/ECompo/Incentive/allincentive';
	import { allbanner } from '$lib/components/Elements/Banners/allbanner';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: IncentiveData = allincentive.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(IncentiveData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={IncentiveData.comp} />
		</div>
		<div>
