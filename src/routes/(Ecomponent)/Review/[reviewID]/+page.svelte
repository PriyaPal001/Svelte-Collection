<script>
	...
	import { allreview } from '$lib/components/ECompo/Review/allreview';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: ReviewData = allreview.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(ReviewData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={ReviewData.comp} />
		</div>
		<div>
