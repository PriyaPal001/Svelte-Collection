<script>
	...
	import { allpv } from '$lib/components/ECompo/Preview/allpv';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: PVData = allpv.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(PVData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={PVData.comp} />
		</div>
		<div>
