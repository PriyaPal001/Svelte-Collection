<script>
	...
	import { allform } from '$lib/components/ECompo/Forms/allform';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: FormData = allform.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(FormData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={FormData.comp} />
		</div>
		<div>
