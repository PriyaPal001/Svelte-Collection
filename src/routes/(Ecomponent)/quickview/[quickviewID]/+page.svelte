<script>
	...
	import { allqv } from '$lib/components/ECompo/Quickview/allqv';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: QuickData = allqv.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(QuickData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={QuickData.comp} />
		</div>
		<div>
