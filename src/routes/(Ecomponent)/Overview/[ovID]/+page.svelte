<script>
	...
	import { allov } from '$lib/components/ECompo/Overview/allov';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: OVData = allov.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(OVData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={OVData.comp} />
		</div>
		<div>
