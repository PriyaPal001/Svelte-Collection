<script>
	...
	import { allcp } from '$lib/components/Navigation/commandPalettes/allcp';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: CPData = allcp.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(CPData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={CPData.comp} />
		</div>
		<div>
