<script>
	...
	import { allsnav } from '$lib/components/Navigation/sideNav/allsnav';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: SnavData = allsnav.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(SnavData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={SnavData.comp} />
		</div>
		<div>
