<script>
	...
	import { allbread } from '$lib/components/Navigation/breadcrums/allbread';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: BreadData = allbread.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(BreadData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={BreadData.comp} />
		</div>
		<div>
