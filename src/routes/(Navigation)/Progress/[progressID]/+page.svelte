<script>
	...
	import { allpb } from '$lib/components/Navigation/progressBars/allpb';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: PBData = allpb.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(PBData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={PBData.comp} />
		</div>
		<div>
