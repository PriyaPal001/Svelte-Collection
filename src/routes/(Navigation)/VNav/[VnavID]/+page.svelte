<script>
	...
	import { allvnav } from '$lib/components/Navigation/verticalNav/allvnav';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: VnavData = allvnav.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(VnavData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={VnavData.comp} />
		</div>
		<div>
