<script>
	...
	import { alltab } from '$lib/components/Navigation/tabs/alltab';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: TabData = alltab.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(TabData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={TabData.comp} />
		</div>
		<div>
