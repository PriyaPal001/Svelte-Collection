<script>
	...
	import { allTexts } from '$lib/components/Forms/Text/alltext';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: TextData = allTexts.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(TextData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={TextData.comp} />
		</div>
		<div>
