<script>
	...
	import { allRgrps } from '$lib/components/Forms/RadioGrp/allrgrp';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: RadioData = allRgrps.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(RadioData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={RadioData.comp} />
		</div>
		<div>
