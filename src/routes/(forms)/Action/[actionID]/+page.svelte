<script>
	...
	import { allActions } from '$lib/components/Forms/Action/allaction';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: actionData = allActions.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(actionData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={actionData.comp} />
		</div>
		<div>
