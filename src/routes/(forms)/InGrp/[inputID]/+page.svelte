<script>
	...
	import { allIngrps } from '$lib/components/Forms/InputGrp/allingrp';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: inGrpData = allIngrps.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(inGrpData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={inGrpData.comp} />
		</div>
		<div>
