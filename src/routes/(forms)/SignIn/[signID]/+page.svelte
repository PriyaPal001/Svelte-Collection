<script>
	...
	import { allSigns } from '$lib/components/Forms/SignIn/allsign';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: SignInData = allSigns.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(SignInData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={SignInData.comp} />
		</div>
		<div>
