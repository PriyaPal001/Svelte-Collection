<script>
	...
	import { allToggles } from '$lib/components/Forms/Toggles/alltoggle';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: ToggleData = allToggles.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(ToggleData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={ToggleData.comp} />
		</div>
		<div>
