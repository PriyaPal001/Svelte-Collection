<script>
	...
	import { allCombos } from '$lib/components/Forms/ComboBox/allcombo';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: comboData = allCombos.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(comboData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={comboData.comp} />
		</div>
		<div>
