<script>
	...
	import { allMenus } from '$lib/components/Forms/SelectMenu/allsmenu';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: selectMData = allMenus.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(selectMData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={selectMData.comp} />
		</div>
		<div>
