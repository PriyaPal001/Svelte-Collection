<script>
	...
	import { allLayouts } from '$lib/components/Forms/FLayout/alllayout';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: layoutData = allLayouts.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(layoutData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={layoutData.comp} />
		</div>
		<div>
