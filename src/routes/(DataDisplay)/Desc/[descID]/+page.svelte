<script>
	...
	import { alldescs } from '$lib/components/DataDisplay/Desc/alldesc';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: DescData = alldescs.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(DescData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={DescData.comp} />
		</div>
		<div>
