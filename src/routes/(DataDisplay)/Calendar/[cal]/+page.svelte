<script>
	...
	import { allcals } from '$lib/components/DataDisplay/cal/allcal';
	$: routeID = $page.params.PageID;
	$: console.log(routeID);
	$: CalData = allcals.filter(n=>n.id===Number(routeID))[0];


	let btnText='Copy Code';
	let copyCode=()=>{
		 navigator.clipboard.writeText(CalData.copy)
            .then(() => {
                btnText = "Copied";
...
<div>
		...
		<div class="mb-4 mt-2 rounded-lg border-2 p-4">
			<svelte:component this={CalData.comp} />
		</div>
		<div>
